...
void floodfillHelper(Cell &c, int direction)
{
    ...
    while (!stack.empty())
    {
        ...
        if (current->y < SIZE - 1)
            neighbors.push_back(Maze::get(current->x, current->y + 1)); // add north neighbor
        if (current->x < SIZE - 1)
            neighbors.push_back(Maze::get(current->x + 1, current->y)); // add east neighbor
        if (current->y > 0)
            neighbors.push_back(Maze::get(current->x, current->y - 1)); // add south neighbor
        if (current->x > 0)
            neighbors.push_back(Maze::get(current->x - 1, current->y)); // add west neighbor
        ...
        for (Cell *neighbor : neighbors)
        {
            ...
            if (!isOpenNeighbor(*current, *neighbor))
                continue;
    ...
    if (c.y + 1 == nextCell->y)
        direction = 0; // north
    else if (c.x + 1 == nextCell->x)
        direction = 1; // east
    else if (c.y - 1 == nextCell->y)
        direction = 2; // south
    else
        direction = 3; // west
...
{
    if (c.y + 1 == neighbor.y && !c.hasNorthWall())
        return true;
    if (c.x + 1 == neighbor.x && !c.hasEastWall())
        return true;
    if (c.y - 1 == neighbor.y && !c.hasSoutWall())
        return true;
    if (c.x - 1 == neighbor.x && !c.hasWestWall())
        return true;
